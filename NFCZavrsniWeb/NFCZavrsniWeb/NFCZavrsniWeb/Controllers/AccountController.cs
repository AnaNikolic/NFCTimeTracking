...
namespace NFCZavrsniWeb.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(AddPhoneNumberViewModel model, string returnUrl)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            }
            var user = UserManager.Users.Where(x => x.PhoneNumber == model.Number).First();

            var role = UserManager.GetRoles(user.Id).First();
            if (role != "SystemAdministrator" && role != "ClientAdministrator")
            {
                ModelState.AddModelError("", "You have no right to access.");
                return View(model);
            }
            var result = await SignInManager.PasswordSignInAsync(user.Email, ConfigurationManager.AppSettings["UserPassword"], false, shouldLockout: false);
        ...
        [AllowAnonymous]
        public async Task<ActionResult> VerifyCode(string provider, string returnUrl = null, bool rememberMe = false)
        {
